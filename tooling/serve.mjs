...
const root=path.resolve('dist'),port=Number(process.env.PORT??4173),host=process.env.HOST??'127.0.0.1';
...
try{await stat(path.join(root,'index.html'));}catch{console.error('No release found. Run npm run build first.');process.exit(1);}
...
server.listen(port,host,()=>console.log(`Arcade ready: http://${host}:${port}`));
